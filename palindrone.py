#!/usr/bin/env python3.5
import unittest


# palindrome code, this is just a test project for pdb (there's a bug here)
#

def digits(x):
    """Convert an integer into a list of digits.

    Args:
        x: The number whose digits we want.

    Returns: A list of the digits, in order of ``x``.
    >>> digits(4586378)
    [4, 5, 8, 6, 3, 7, 8]
    """
    # NOTE: import is done in the method
    # import pdb
    # pdb.set_trace()

    digs = []
    while x != 0:
        div, mod = divmod(x, 10)
        digs.append(mod)
        # x = mod  # THIS IS THE BUG
        x = div
    return digs


def is_palindrome(x):
    """Determine if an integer is a palindrome.

    Args:
        x: The number to check for the palidrom-o-city.

    Returns: True if the digits of ``x`` are a palindrome, else false.

    >>> is_palindrom(1234)
    False
    >>> is_palindrom(2468642)
    True
    """
    digs = digits(x)
    for f, r in zip(digs, reversed(digs)):
        if f != r:
            return False
    return True


class Tests(unittest.TestCase):
    """ Test for the ```is_palindrome()``` function."""

    def test_negative(self):
        "Check that it returns false correctly."
        self.assertFalse(is_palindrome(1234))

    def test_positive(self):
        "Check that it returns true correctly."
        self.assertTrue(is_palindrome(1230321))

    def test_single_digit(self):
        "Check that it works for single digit numbers."
        for i in range(10):
            self.assertTrue(is_palindrome(i))

if __name__ == '__main__':
    unittest.main()
